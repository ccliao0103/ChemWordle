# ChemWordle 題庫擴充 — batch4 候選(150 題)
...
- 全部 150 題都通過檢查:**無重複、長度合格(5-6)、無複數**
...
| TOLYL | 甲苯基 | organic | ★★★ |   | CH₃C₆H₄–,甲苯少一個氫。 | CH₃C₆H₄–; toluene minus one hydrogen. |
...
| IMIDE | 醯亞胺 | organic | ★★★ |   | 兩個醯基接同一個氮,如聚醯亞胺(PI)薄膜。 | Two acyl groups on one nitrogen, e.g., polyimide films. |
| OXIME | 肟 | organic | ★★★ |   | 含 C=N–OH,醛酮與羥胺的反應產物。 | Contains C=N–OH; from aldehydes/ketones plus hydroxylamine. |
| AZINE | 氮雜環 | organic | ★★★ |   | 含氮的六員芳香環,如吡啶。 | Six-membered aromatic ring containing nitrogen, e.g., pyridine. |
| ACETAL | 縮醛 | organic | ★★ |   | 醛與兩分子醇的產物,醣類環化的關鍵。 | From an aldehyde plus two alcohols; key to sugar ring formation. |
| KETAL ⚠️ | 縮酮 | organic | ★★★ |   | 酮與兩分子醇反應的產物。 | The product of a ketone reacting with two alcohols. |
| LACTAM | 內醯胺 | organic | ★★★ |   | 環狀醯胺,盤尼西林的 β-內醯胺環。 | A cyclic amide; the β-lactam ring in penicillin. |
...
| INDOLE | 吲哚 | organic | ★★★ |   | 苯環併吡咯;色胺酸與植物荷爾蒙的骨架。 | Benzene fused to pyrrole; core of tryptophan and auxins. |
...
| ANOMER ⚠️ | 異頭物 | organic | ★★★ |   | 醣環化後 C1 位的立體異構物(α / β)。 | Stereoisomers at C1 of a cyclic sugar (α or β). |
| EPIMER | 差向異構物 | organic | ★★★ |   | 只有一個不對稱碳構型不同的立體異構物。 | Stereoisomers differing at exactly one chiral centre. |
...
| MALEIC ⚠️ | 順丁烯二酸的 | organic | ★★★ |   | 順式丁烯二酸,與反式的富馬酸互為幾何異構物。 | cis-butenedioic acid; geometric isomer of fumaric acid. |
...
| TRIOSE | 三碳醣 | bio | ★★★ |   | 三個碳的單醣,糖解作用的中間產物。 | A three-carbon sugar; an intermediate in glycolysis. |
...
| CERMET | 金屬陶瓷 | mat | ★★★ |   | 陶瓷與金屬的複合材料,兼具硬度與韌性。 | A ceramic–metal composite combining hardness and toughness. |
...
| SPINEL | 尖晶石 | mat | ★★★ |   | MgAl₂O₄,也是一整類立方氧化物結構的統稱。 | MgAl₂O₄; also names a whole family of cubic oxide structures. |
...
| BARITE | 重晶石 | mat | ★★★ |   | BaSO₄,密度高,用作鑽井泥漿加重劑。 | BaSO₄; dense, used as a drilling-mud weighting agent. |
...
| CURIUM | 鋦 | element | ★★★ |   | 人造錒系元素,以居禮夫婦命名。 | A synthetic actinide named after the Curies. |
| ERBIUM | 鉺 | element | ★★★ |   | 稀土元素,摻鉺光纖放大器(EDFA)的核心。 | Rare earth at the heart of erbium-doped fiber amplifiers (EDFA). |

## 9. 物理化學(11 題)

| 英文 | 中文 | 類別 | 難 | 訪客 | zh_description | en_description |
|---|---|---|---|---|---|---|
| PHONON | 聲子 | phys-chem | ★★★ |   | 晶格振動的量子,決定熱傳導與比熱。 | The quantum of lattice vibration; governs heat conduction and specific heat. |
...
| ISOBAR | 等壓 / 同量異位素 | phys-chem | ★★★ |   | 壓力相同的狀態;核物理中指質量數相同的核種。 | A constant-pressure condition; in nuclear physics, nuclides of equal mass number. |
...
| 英文 | 中文 | 類別 | 難 | 訪客 | zh_description | en_description |
|---|---|---|---|---|---|---|
| ADDUCT | 加成物 | reaction | ★★★ |   | 兩分子直接結合而不失去任何原子的產物。 | A product formed when two molecules combine with no atoms lost. |
...
| 英文 | 中文 | 類別 | 難 | 訪客 | zh_description | en_description |
|---|---|---|---|---|---|---|
| ELUENT | 沖提液 | gen | ★★★ |   | 層析中用來帶動樣品通過管柱的溶劑。 | The solvent that carries a sample through a chromatography column. |
...
| IODATE | 碘酸鹽 | gen | ★★★ |   | 含 IO₃⁻ 的鹽,食鹽加碘多用碘酸鉀。 | Salts of IO₃⁻; potassium iodate is used to iodize salt. |
...
| LABILE | 不穩定的 | gen | ★★★ |   | 容易變化或被取代,配位化學中與 inert 相對。 | Easily changed or substituted; the opposite of inert in coordination chemistry. |
...
| TITER | 效價 | analytical | ★★★ |   | 溶液濃度的度量,常用於抗體或標準液。 | A measure of solution concentration, common for antibodies and standards. |
...
| MOLAL | 重量莫耳濃度的 | analytical | ★★★ |   | 每公斤溶劑所含溶質莫耳數(m)。 | Moles of solute per kilogram of solvent (m). |
